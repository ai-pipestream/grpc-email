...
body parts, attachments, status. One JVM: Jakarta Mail for `.eml`, Apache POI
HSMF for `.msg`. It is not PipeStream core and there is no Python in the
serving path.

Two properties define the service:

1. **The live stream is the product.** The server emits the envelope the
   moment the header block has arrived. On a chunked `.eml` upload that is
   *before the client has finished sending*. Then every body part and
   attachment as the MIME walk reaches it. `ParseStatus` is a trailer of
...
```mermaid
flowchart LR
    client[Client] -->|"options, then EmailChunk stream"| svc[EmailParseService]
    svc --> sniff[EmailSniffer<br/>format from bytes]
    sniff -->|EML| eml[EmlParser<br/>Jakarta Mail]
    sniff -->|MSG| msg[MsgParser<br/>POI HSMF MAPIMessage]
    eml --> events[typed events]
    msg --> events
    events --> fold{emit_document?}
    fold -->|yes| doc[EmailDocumentFold<br/>Document event]
    fold -->|no| out
    doc --> out[ParseEmailResponse stream:<br/>EmailInfo, BodyPart, Attachment,<br/>Document, ParseStatus]
    out --> client
```
...
`ai.pipestream.email.v1.EmailParseService`, defined in
...
typed events stay exactly as they were, so a client can diff the two streams,
