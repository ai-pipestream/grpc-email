...
| `content_type` option | `source_meta.extra["email.declared_content_type"]`. Advisory only, and recorded where it cannot be mistaken for `origin.mimetype`, which stays the type detected from the bytes. The two disagreeing is a fact worth keeping, not one to resolve here |
...
| envelope, again | `Document.source_meta` (`DocumentMeta`): subject as `title`, origination date as `created` in ISO 8601, and `extra` carrying `email.from` / `to` / `cc` (rendered mailboxes joined with `", "`, because `extra` is `map<string, string>`) plus `email.message_id`, `email.in_reply_to` (every id, space separated), `email.references`, and for `.msg` `email.conversation_topic` / `email.conversation_index`. This is the cross-collector slot: a consumer asking "who wrote this and when" reads the same fields whatever the source was, instead of learning one collector's custom-field vocabulary. An envelope with nothing in it produces no `source_meta` at all |
